...
class Robot:
    ...
    def Color(self):
        if self.ProgramType == 0:
            ...
            return 0, 255, 0
        elif self.ProgramType == 3:
            return 0, 0, 255
        elif self.ProgramType == 4:
            return 0, 255, 255
        elif self.ProgramType == 5:
            return 255, 0, 255
    ...
    def Decision_Melnyk(self):
        if self.IsItemHere():
            return 'Take'
        view = self.View()
        if (view > 0) or (random.random() < 0.1):
            if random.randint(0, 1) == 0:
                return 'Turn Left'
            else:
                return 'Turn Right'
        elif (view == -1) or (0 < view < 11 and random.random() < 0.1):
            if self.Energy > 0:
                return 'Shoot'
            else:
                if random.randint(0, 1) == 0:
...
def PrintRobotsCount():
    global Game
    s = ''
    NSurvivedProgs = 0
    for i_prog in range(N_PROGRAMS):
        N = 0
        for rbt in Robots:
            if rbt.ProgramType == i_prog:
                N += 1
        s += '[' + str(i_prog) + ']:' + str(N) + '\t'
        if N > 0:
            NSurvivedProgs += 1
    print(s)
    if NSurvivedProgs == 1:
        print('ВЫЖИЛА ПОПУЛЯЦИЯ ' + str(Robots[0].ProgramType))
        Game = False
    elif NSurvivedProgs == 0:
